...
def Acq_Cost(L):
    """
    Compute acquisition cost for a lander object L.
    Works for both scalars and numpy arrays.
    """
    # Each CER returns (dev, prod) → arrays or scalars
    dev_str, prod_str = STR(L.STR)
    dev_av, prod_av   = AVIO(L.AVIO)
    dev_oth, prod_oth = OTH(L.OTH)
    dev_pow, prod_pow = POW(L.POW)
    dev_th, prod_th   = THER(L.THER)
    dev_pr, prod_pr   = PRPLN(L.PRPLSN)

    # Sum dev + prod across all subsystems
    total = (dev_str + prod_str +
             dev_av  + prod_av  +
             dev_oth + prod_oth +
             dev_pow + prod_pow +
             dev_th  + prod_th  +
