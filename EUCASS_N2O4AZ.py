...
def routine_Isaji_N2O4_MLR(mp, dv, Isp):
    ...
    a = lander.STR = ESR.STR_star(X)
    c = lander.POW = ESR.POW_star(X)
    d = lander.AVIO = ESR.AVIO_star(X)
    e = lander.THER = ESR.THER_star(X)
    f = lander.OTH = ESR.OTH_star(X)
...
def routine_stat_MLR_iter(mp, dv, Isp):
    ...
    while er > tol:
        X = np.array([lander.md, mp, lander.mprop, Isp, dv]).reshape(1,-1)
        a = lander.STR = ESR.STR_star(X)
        b = lander.PRPLSN = ESR.PROP_star(X)
        c = lander.POW = ESR.POW_star(X)
        d = lander.AVIO = ESR.AVIO_star(X)
        e = lander.THER = ESR.THER_star(X)
        f = lander.OTH = ESR.OTH_star(X)
