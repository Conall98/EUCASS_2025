...
plt.scatter(x, y, color="blue", s=30, label="Data")
...
y_preds = f1_unc(x_fit)
plt.scatter(x_fit[::5], y_preds[::5], color = "orange", label = "pred")
plt.xlim(0, max(x_fit)+10)
plt.ylim(0, max(y_preds)+3*std_curve[-1:])
plt.legend()
